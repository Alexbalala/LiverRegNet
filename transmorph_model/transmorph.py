...
from swin_model.swin_transformer import SwinTransformer3D
...
class dualAtt_24(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.up2(x, f3)
        x = self.up3(x, f4)
        x = self.up4(x, f5)  #8,16,32,96,96
        # flow = self.reg_head(x)  #8,3,32,96,96
        # out = self.spatial_trans(source, flow)
        flow = x
        flow = self.path1_block1_bn(flow)
        flow = self.leakrelu(flow)
        flow = self.cbam_7(flow)
        # flow = self.path1_block3_NLCross(flow, flow)
        # out = self.spatial_trans(source, flow)
...
def AttentionReg():
    ...
    model = dualAtt_24(CONFIGS['TransMorph'])
